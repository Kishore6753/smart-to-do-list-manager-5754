...
export function useCategories() {
  /** Fetch and manage categories list state */
...
export function CategoriesPanel() {
  /** Sidebar-like categories manager */
      ...
      {loading ? (
        ...
        <ul className="list" aria-label="Category list">
          {categories.map((c) => {
            const id = c.id ?? c._id;
            return (
              <li key={id} className="task-item" style={{ gridTemplateColumns: "1fr auto" }}>
                <div>
                  <div className="task-title">{c.name}</div>
                  <div className="task-meta">
                    <span className="badge">
                      <span className="dot" style={{ background: c.color || "#ccc" }} />
                      {c.color || "No color"}
                    </span>
                  </div>
                </div>
                <div className="row">
                  <button type="button" className="btn danger" onClick={() => remove(id)} aria-label={`Delete ${c.name}`}>
                    Delete
                  </button>
                </div>
              </li>
...
// PUBLIC_INTERFACE
export function TasksPanel({ initialQuery = "" }) {
  /** Tasks panel with quick add, filters, list, drag-and-drop, reminders */
  ...
  const onUpdate = async (id, payload) => {
    ...
    setTasks((prev) => prev.map((t) => (String(t.id ?? t._id) === String(id) ? updated : t)));
  };
  ...
  const onDrop = async (e, overId) => {
    ...
    const curr = [...tasks];
    const fromIndex = curr.findIndex((t) => String(t.id ?? t._id) === String(fromId));
    const overIndex = curr.findIndex((t) => String(t.id ?? t._id) === String(overId));
      ...
      {loading ? (
        ...
        <ul className="list" aria-label="Tasks list">
          {filtered.map((t) => {
            const id = t.id ?? t._id;
            const category =
              categories.find((c) => String(c.id ?? c._id) === String(t.category_id)) || null;
            return (
              <li
                key={id}
              ...
              >
                ...
                <div>
                  <div className="task-title">
                    ...
                    {category && (
                      <span className="badge" title={category.name}>
                        <span className="dot" style={{ background: category.color || "#ccc" }} />
                        {category.name}
                      </span>
                    )}
                  </div>
                  ...
                  <div className="task-meta">
                    ...
                    {t.reminder_at && <span style={{ marginLeft: 8 }}>Reminder: {fmtDate(t.reminder_at)}</span>}
                  </div>
...
function EditTaskModal({ task, categories, onClose, onSave }) {
  ...
  const [reminder, setReminder] = useState(task.reminder_at ? toLocalInput(task.reminder_at) : "");
  ...
  return (
    <div
      ...
      onClick={onClose}
    >
      <div className="card" style={{ width: "min(640px, 96vw)" }} onClick={(e) => e.stopPropagation()}>
